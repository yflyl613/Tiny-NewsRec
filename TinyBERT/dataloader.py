...
class DataLoaderTrain(IterableDataset):
    ...
    def _process(self, batch):
        ...
        history_index_batch, candidate_index_batch, label_batch = [], [], []
        batch_news_feature = {}
        ...
        for line in batch:
            ...
            click_docs_index = []
            for doc in click_docs[-self.user_log_length:]:
                if doc not in batch_news_feature:
                    batch_news_feature[doc] = len(batch_news_feature) + 1
                click_docs_index.append(batch_news_feature[doc])

            click_docs_index, _ = self.pad_to_fix_len(
                click_docs_index, self.user_log_length)

            pos_index, neg_index = [], []
            for doc in sess_pos:
                if doc not in batch_news_feature:
                    batch_news_feature[doc] = len(batch_news_feature) + 1
                pos_index.append(batch_news_feature[doc])

            for doc in sess_neg:
                if doc not in batch_news_feature:
                    batch_news_feature[doc] = len(batch_news_feature) + 1
                neg_index.append(batch_news_feature[doc])

            label = random.randint(0, self.npratio)
            candidate_news_index = neg_index[:label] + \
                pos_index + neg_index[label:]

            history_index_batch.append(click_docs_index)
            candidate_index_batch.append(candidate_news_index)
            label_batch.append(label)

        batch_news_nindex = self.trans_to_nindex(
            list(batch_news_feature.keys()))
        batch_news_feature = self.news_combined[batch_news_nindex]

        if self.enable_gpu:
            batch_news_feature = torch.LongTensor(batch_news_feature).cuda()
            history_index_batch = torch.LongTensor(history_index_batch).cuda()
            candidate_index_batch = torch.LongTensor(
                candidate_index_batch).cuda()
            label_batch = torch.LongTensor(label_batch).cuda()
        else:
            batch_news_feature = torch.LongTensor(batch_news_feature)
            history_index_batch = torch.LongTensor(history_index_batch)
            candidate_index_batch = torch.LongTensor(candidate_index_batch)
            label_batch = torch.LongTensor(label_batch)

        return batch_news_feature, history_index_batch, candidate_index_batch, label_batch
